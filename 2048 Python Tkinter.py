...
def create_phantom_tiles(previous_board: list[list[int]], board: list[list[int]], direction: str):
    ...
    global font_size
    '''
    Creates "phantom tiles" which mark where the existing tiles should end up after their sliding
    animation is over by comparing the square positions on the board before & after moving.  
    '''
    rows = len(board)
    cols = len(board[0])
...
# move_down and move_up functions use matrix transposition and the 
# existing move_left & move_right functions to transpose the matrix.
# Time complexity is O(n^2) instead of O(n) if the manipulation of columns was direct, however
# the usage of already existing functions makes it easier to add onto in the future.
def move_down(event):
    global can_move
    if can_move == False: return
    global board
